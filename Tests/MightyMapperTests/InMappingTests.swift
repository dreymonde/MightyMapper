...
struct Nest3: InMappableWithContext {
    ...
    init<Source>(mapper: ContextualInMapper<Source, String, Context>) throws {
...
struct Test9: InMappableWithContext {
    let nest: Nest3
    init<Source>(mapper: ContextualInMapper<Source, String, TestContext>) throws {
        self.nest = try mapper.map(from: "nest")
    }
}
...
struct Test13: InMappable {
    let nests: [Nest2]
    init<Source>(mapper: PlainInMapper<Source>) throws {
...
struct DictTest: InMappable {
    ...
    let nests: [DictNest]
    let null: Bool?
    init<Source>(mapper: InMapper<Source, String>) throws {
        self.int = try mapper.map(from: "int")
...
extension Date : InMappableWithContext {
    ...
    public init<Source>(mapper: PlainContextualInMapper<Source, Context>) throws {
        let interval: TimeInterval = try mapper.map()
        switch mapper.context {
        case .timeIntervalSince1970:
            self.init(timeIntervalSince1970: interval)
...
class InMapperTests: XCTestCase {
    ...
    func testPrimitiveMapping() throws {
        let primitiveDict: [String : Any] = ["int": 5, "string": "String", "double": 7.8, "bool": true]
    ...
    func testFailNoValue() {
        let dict: [String : Any] = ["string": "Rio-2016"]
    ...
    func testFailWrongType() {
        let dict: [String : Any] = ["rio-2016": 2016]
        XCTAssertThrowsError(try Test3(from: dict)) { error in
    ...
    func testFailRepresentAsArray() {
        let dict: [String : Any] = ["ints": false]
    ...
    func testEnumFail() {
        let dict: [String : Any] = ["project": "swansea"]
    ...
    func testContextInference() throws {
        let peach: [String : Any] = ["nest": ["peach-int": 207]]
        _ = try Test9(from: peach, withContext: .peach)
    }
    
    func testArrayMappingWithContext() throws {
        let oranges: [[String : Any]] = [2, 0, 1, 6].map({ ["orange-int": $0] })
        let dict: [String : Any] = ["nests": oranges]
    ...
    func testDeep() throws {
        let deepDict: [String : Any] = ["deeper": ["stillDeeper": ["close": ["gotcha": "Ukrainian Gold Medal"]]]]
        let deep = try Test12(from: deepDict)
        XCTAssertEqual(deep.hiddenFar, "Ukrainian Gold Medal")
    }
    
//    func testFlatArray() throws {
//        let dict: [String : Any] = [["int": 15], ["int": 21]]
...
//    }
    ...
    func testAdvancedEnum() throws {
        ...
        let fireMap: [String : Any] = try fire.map()
